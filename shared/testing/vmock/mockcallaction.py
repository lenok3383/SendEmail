...
:Status: $Id: //prod/main/_is/shared/python/testing/vmock/mockcallaction.py#2 $
:Author: vburenin
"""

from shared.testing.vmock import matchers, mockerrors
...
class MockCallAction(object):
    ...
    def _is_times_limit(self):
        ...
        """
        return self.__max_times == self.__calls_counter and \
               self.__max_times > 0
    ...
    def _get_result(self, *args, **kwargs):
        ...
        if self.__raise_call_error:
            self.obj._mc._raise_error(
                    mockerrors.UnexpectedCall('Unexpected call catched!'))
    ...
    def _compare_args(self, args, kwargs):
        ...
        if self.args and len(self.args) == 1 and \
                         isinstance(self.args[0], matchers._AnyArgsMatcher):
            return True

        # Check if there are any arguments matcher.
        if args and len(args) == 1 and \
                         isinstance(args[0], matchers._AnyArgsMatcher):
            return True
    ...
    def anytimes(self):
        ...
        assert self.is_ordered, 'Must be non-ordered to call any times'
        self.__max_times = 0
        self.__min_times = 0
        return self
