...
:Status: $Id: //prod/main/_is/shared/python/util/test/test_lrucache.py#4 $
:Author: vburenin
"""

import unittest2

from shared.util.lrucache import LRUCache, LRUTimeCache, CacheExpiredError
...
class TestLRUTimeCache(unittest2.TestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual((1, 0, 1, 0), cache.stats())

    def test_get_ttl_failed(self):
        cache = LRUTimeCache(5, -1)
        cache.put(1, '1')
        self.assertRaises(CacheExpiredError, cache.get, 1)
        self.assertEqual((0, 1, 0, 1), cache.stats())

    def test_get_keyerror(self):
        cache = LRUTimeCache(5, 100000)
        cache.put(1, '1')
        self.assertRaises(KeyError, cache.get, 2)
        self.assertEqual((0, 1, 1, 0), cache.stats())
